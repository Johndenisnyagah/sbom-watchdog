...
**`report.py` is deleted.** It shipped empty through packaging. Dead weight in a security tool invites the question of what else is not doing anything, and an empty module in the action's file list is a claim the tool does not honour. It returns when build-order step 5 builds `SECURITY-INVENTORY.md` and it has something to do.
