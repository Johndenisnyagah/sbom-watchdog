...
└── report.py   SECURITY-INVENTORY.md and the dated SBOM history
```
...
`packaging/real_0117.json` is the one observed sample rather than a reconstruction: genuine Grype 0.117.0 output, absolute paths scrubbed and nothing else altered. It lives outside `tests/` because the action bundles it, and there is exactly one copy — two would let the action's self-check and this suite assert against different bytes, which is how a packaged action passes its own check while the parser has drifted. `build_fixtures.py` does not generate it. It is the only thing that would catch a Grype schema change before a red workflow run does.
...
`sboms/YYYY-MM-DD.json` is written every day the scan runs and is never skipped for being unchanged. The counter-argument is real — dependencies change rarely, so most days the file is near-identical to the day before and a year of them is 365 near-duplicates. It loses anyway, because a missing file has to mean exactly one thing. If absence could mean either "no scan ran" or "nothing changed", the trail cannot distinguish a stable project from an abandoned one, which is the single question it exists to answer. Scheduled workflows are disabled after 60 days of inactivity, so going quiet is a realistic ending rather than a hypothetical one, and this is the same reasoning that makes `last_seen` advance daily.

Deduplication would also need defining, which is a second reason against it: the CycloneDX document carries a `serialNumber` and a `metadata.timestamp` that change on every run, so byte-comparison never suppresses anything. Measured on two scans of an identical tree seconds apart, `components` was identical and only those two fields differed. Suppressing would therefore mean comparing `components` and explaining that rule to whoever reads the trail.

The first scan of a day writes that day's SBOM and later scans the same day leave it alone. Syft stamps a fresh `serialNumber` and `metadata.timestamp` on every run, so rewriting produces a commit whose entire content is a changed UUID, and it silently changes what the file means: the day's record becomes the last scan of that day, with nothing saying which. The run history already records that a later scan happened. This is a different rule from the one above and does not weaken it — a day with no file still means exactly one thing, that no scan ran.

The inventory is the opposite and is rewritten whenever it would differ, including to undo a hand edit, because it describes current state rather than a dated snapshot. Writing content identical to what is on disk is a no-op rather than a rewrite, and both writers report which happened: `should_commit` has to be false when a repeat scan leaves everything as it was.

`SECURITY-INVENTORY.md` is written for a reader who is not a developer, and the register is part of the product in the same way the issue body is. No `purl`, no `fix_state`, no `not-fixed`, no `CycloneDX`; a test asserts that vocabulary never reaches it. Severity is explicitly described as the published rating rather than an assessment of this project's exposure, because overstating that is the easiest way for a compliance document to mislead the person relying on it. Findings with no available fix are counted separately, because a package awaiting an upgrade is scheduled work and a package with no published fix is a decision about whether to keep depending on it — different answers to give an auditor.

**`should-commit` and `state-changed` are two facts, not one.** `state_changed` answers "did the vulnerability state move". `should_commit` answers "is there anything to write", which is also true when nothing moved but a dated SBOM or the inventory was produced. Gating the commit on the first left a repository with no findings unable to commit its audit trail at all: `findings` stays empty, so once the tooling settles the document is identical every day and `state_changed` is false forever. That inverted the write-every-day decision — a missing dated SBOM was supposed to mean "no scan ran that day" and had quietly come to mean "or the project was clean", which is the readership the trail exists for. Both outputs are kept, because "a finding appeared or went away" is a distinct and useful signal from "there is a file to commit", and overloading one name would lose it.
...
**The per-package line has three shapes, because they lead to three actions.** With a fix everywhere, the question is which version, answered by the highest. With no fix anywhere, the reader is deciding about the dependency rather than about a version, and the useful fact is that one decision closes all of them — without it, two issues against the same abandoned package read as two separate decisions, each carrying the same "replace it or accept the risk" paragraph and neither acknowledging the other. Mixed is the dangerous shape and was silently wrong: it took the fixable wording, which implies an upgrade covers everything, when upgrading closes some and leaves the rest. It now states and counts both halves. Counts cover every finding for the package in the scan, not only those above the threshold — a below-threshold finding is still resolved by replacing the package, and the count has to be honest about the work.

**A finding with no fix is a different kind of work item.** Rendered for a human for the first time after 53 real findings across two testbeds, all of which reported `fixed`, the no-fix body read as a broken upgrade notice: `**Fixed in:** no fixed version has been published yet` is a bold label promising a version and then negating itself, and it left the reader to work out unaided that upgrading was not on the table. The lead-in now states the situation — `**No fix is available.**`, `**No fix is planned.**` — and names the decision: replace the dependency, or accept the risk and record why. The `Fix state` table row is dropped there, since `not-fixed` is scanner vocabulary restating the prose in a second dialect. A `no-fix` label makes them filterable, because a dependency-replacement decision is not a version bump. And the footer no longer says the finding stays recorded "until the dependency is no longer reported as vulnerable": pycrypto's last release was 2013, so that never happens and the sentence implies waiting is a strategy. Where nothing is coming, it says until the dependency is removed or replaced.

**Issue bodies are single-line paragraphs.** GitHub renders a newline inside a paragraph as a line break, so hard-wrapped prose breaks at whatever column the source happened to use — and substituting a phrase of a different length leaves it visibly ragged.
...
**GitHub evaluates every expression in `action.yml`, descriptions included.** `${{ secrets.GITHUB_TOKEN }}` written as an example inside an input's `description` is not documentation to the template parser; it is an expression, and the `secrets` context does not exist inside a composite action. The manifest failed to load before a single step ran. Write context names as plain text in prose — `secrets.GITHUB_TOKEN`, no braces — and keep expressions to slots that are meant to hold them.

**`tests/test_manifest.py` guards the manifest's shape, not one bad name.** Expressions only in slots meant to hold them, contexts only from the composite-safe set, no keys a composite action rejects, every `run` step declaring a shell, every referenced input declared. It parses `action.yml` by hand rather than with PyYAML — a YAML parser discards exactly the distinction under test, which is *where in the file* an expression appears. Verified by mutation: restoring the shipped bug fails three of them independently.
...
**`report.py` is back, with work to do.** It was deleted while empty rather than shipped as dead weight, and returns now that build-order step 5 exists: `SECURITY-INVENTORY.md` and the dated SBOM history.
